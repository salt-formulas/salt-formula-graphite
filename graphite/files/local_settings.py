...
DATABASES = {
    'default': {
        'ENGINE': "django.db.backends.{% if server.database.engine == 'postgresql' %}postgresql_psycopg2{% else %}{{ server.database.engine }}{% endif %}",
        'NAME': '{{ server.database.name }}',
        'USER': '{{ server.database.user }}',
        'PASSWORD': '{{ server.database.password }}',
        'HOST': '{{ server.database.host }}',
        'PORT': '{% if server.database.engine == 'postgresql' %}5432{% else %}3306{% endif %}',
    }
}
...
{%- if server.carbon_links is defined %}
CARBONLINK_HOSTS = [{% for link in server.carbon_links %}'{{ link.host }}:{{ link.port }}'{% if not loop.last %}, {% endif %}{% endfor %}]
{%- endif %}
...
{%- else %}
SECRET_KEY = 'fsdk43lmr3rl2mdwqldm32lkrm32tk43,dw;32m3ro04'
{%- endif %}
